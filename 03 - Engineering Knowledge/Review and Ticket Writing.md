...
## Ticket text
- A ticket description states the requirement, never the completion state. Never mark acceptance criteria Done / Partly done / Not done, never add "verified" or "implemented" notes, and never shift the problem statement into the past tense ("the app had nothing to call") when reconciling a ticket against the code. Status lives in the workflow; a description that reads as finished misleads whoever picks the ticket up next. Still-owed items stay as plain acceptance criteria.
